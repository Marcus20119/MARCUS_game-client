...
import { actionRestoreUser } from '~/store/admin/admin.action';
import { handleHideAdminModal } from '~/store/admin/admin.slice';
import { IRootState } from '~/store/rootReducer';

const ModalRestoreUser = () => {
  const dispatch = useDispatch();
  const { showRestoreUserModal, loadingRestoreUser, selectedUserData } =
      ...
      >
        <div className="flex flex-col gap-6 w-full">
          ...
          <div className="grid grid-cols-2 gap-3 w-full">
            <ButtonPrimary
              isSubmitting={loadingRestoreUser}
              additionalClass="!bg-gray-300 !text-black"
              onClick={() => dispatch(actionRestoreUser(selectedUserData.id))}
            >
              Restore
            </ButtonPrimary>
            <ButtonPrimary
