...
const ModalHardDeleteUser = () => {
  const dispatch = useDispatch();
  const { showHardDeleteUserModal, loadingHardDeleteUser, selectedUserData } =
    useSelector((state: IRootState) => state.admin);

  return (
    <ModalBase
      visible={showHardDeleteUserModal}
      ...
      >
        <div className="flex flex-col gap-6 w-full">
          ...
          <div className="grid grid-cols-2 gap-3 w-full">
            <ButtonPrimary
              isSubmitting={loadingHardDeleteUser}
              additionalClass="!bg-gray-300 !text-black"
              onClick={() =>
                dispatch(
                  actionDeleteUser({ id: selectedUserData.id, type: 'hard' })
                )
              }
            >
              Delete
...
export default ModalHardDeleteUser;
